...
if FLAGS.eval:
    x_raw, y_test = data_helpers.load_financing_corpus(FLAGS.model_path, FLAGS.test_data_file, train_mode=False)
    y_test = data_helpers.convert_one_hot(FLAGS.model_path, y_test)
...
y_labels,y_label_to_index = json.load(open(os.path.join(FLAGS.model_path, MAPPING_FILE_NAME), "r"))
...
with graph.as_default():
    ...
    with sess.as_default():
        ...
        for i, x_test_batch in enumerate(batches):
            ...
            #print("{} --> probability:{}".format(x_raw[i], ' 'join({y_labels[idx]:p for idx,p in  enumerate(batch_probability)}))
            print(x_raw[i])
            p_str = ''
            for idx,p in enumerate(batch_probability[0]):
                print('{}:{}'.format(y_labels[idx], p))
            print("---------")
